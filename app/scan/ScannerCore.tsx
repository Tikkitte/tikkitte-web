...
import type { FrameDecoder } from '@/lib/scan/decode'
...
export default function ScannerCore({ decoder }: { decoder: FrameDecoder }) {
  ...
  async function doScan() {
    ...
    try {
      const rawValue = await decoder.detect(canvas, ctx)
      if (rawValue) {
        await handleQRCode(rawValue)
        return
      }
    } catch {
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      const unsupportedMessage = decoder.checkUnsupported?.()
