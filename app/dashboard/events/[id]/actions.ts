...
type SendEventAlertInput = {
  eventId: string
  subject: string
  body: string
}

type SendEventAlertResult =
...
export async function sendEventAlert(input: SendEventAlertInput): Promise<SendEventAlertResult> {
  const eventId = input.eventId.trim()
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) {
    return { ok: false, message: 'You must be signed in to message attendees.' }
  }
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/send-event-alert`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${session.access_token}`,
  ...
  if (!response.ok || typeof result?.sent_count !== 'number') {
    return {
      ok: false,
      message: result?.message ?? 'Failed to send attendee message.',
