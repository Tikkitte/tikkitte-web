    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      ...
      <div className="p-6">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800">
                      <th className="text-left py-3 pr-4 font-medium text-gray-500 dark:text-gray-400">Amount</th>
                      ...
                      <th className="text-right py-3 pl-4 font-medium text-gray-500 dark:text-gray-400">Paid on</th>
                    </tr>
                  </thead>
                  <tbody>
                    {payments.map((p) => (
                      <tr key={p.reference} className="border-b border-gray-50 dark:border-gray-800/50 last:border-0">
                        ...
                        <td className="py-3 px-4 text-gray-500 dark:text-gray-400 font-mono text-xs">{p.reference}</td>
                        <td className="py-3 px-4 text-gray-600 dark:text-gray-300">{p.ticketLabel}</td>
                        <td className="py-3 px-4 text-right text-gray-900 dark:text-white font-semibold">{p.quantity}</td>
                        <td className="py-3 pl-4 text-right text-gray-500 dark:text-gray-400 text-xs whitespace-nowrap">{p.paidAt}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab === 'attendees' && (
          <>
            {attendees.length === 0 ? (
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-8">No tickets sold yet.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                  ...
                  <tbody>
                    {attendees.map((a) => (
                      <tr key={a.id} className="border-b border-gray-50 dark:border-gray-800/50 last:border-0">
                        <td className="py-3 pr-4 font-medium text-gray-900 dark:text-white">{a.name}</td>
                        <td className="py-3 px-4 text-gray-500 dark:text-gray-400">{a.email}</td>
                        <td className="py-3 px-4 text-gray-600 dark:text-gray-300">{a.ticketLabel}</td>
