    ...
    <section className="mx-auto max-w-[1280px] px-5 pt-[clamp(72px,10vh,120px)] lg:px-14">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div>
          ...
          <h2 className="mt-2 font-anton text-[clamp(36px,5vw,68px)] uppercase leading-[1.02] text-[#191917]">Hot right now</h2>
        </div>
      ...
      <div className="mt-11 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {events.slice(0, 3).map((event) => (
          ...
          >
            <div className="relative h-[200px] bg-[#F4F2EC]">
              {event.image?.[0] && <Image src={event.image[0]} alt={event.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 33vw" />}
            </div>
            <div className="flex flex-1 flex-col gap-2 p-[22px]">
              ...
              <div className="font-anton text-2xl uppercase leading-[1.1]">{event.name}</div>
